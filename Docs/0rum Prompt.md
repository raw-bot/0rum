...
- **Install 0rum LAST**, just before the hand-off — so the shell reload doesn't bork our session midway.
...
**Q5 — Reflection cadence.** 0rum will reflect on outcomes every N closed trades and propose ONE variable to change. How many trades per reflection? (Default: `5`.)

When all five answers are in, **read them back as a strategy** in plain English:

> "Your strategy: trade `{asset}`. Success is `+{return}%` in 30 days with at least Sharpe `{sharpe}`. Failure is `{drawdown}%` drawdown. Every `{reflection}` closed trades, 0rum will look at the outcomes and change exactly one variable. Confirm to lock it in."

**Wait** for confirmation. Then write `~/0rum-trading/state/goal.yaml` from those answers:
...
Print: `✓ Strategy locked. ~/0rum-trading/state/goal.yaml`

---

## PHASE 3: SCAFFOLD THE WORKER LOCALLY

Say: "Step 3 of 7: Building the trading worker. This is the dumb piece that pulls market data, takes paper trades, and logs every outcome. The smart piece — 0rum — comes later."

Create the directory tree at `~/0rum-trading`:
...
│   ├── reflect.py        ← deterministic fallback (used until 0rum takes over)
...
- **`--fallback`** — deterministic rule, used in Phase 5 before 0rum is installed. If realised return < target → loosen `entry.threshold` by 2. If drawdown > max → tighten `stop_loss_pct` by 0.2. Always changes exactly ONE variable. Bumps version, saves prior to `state/history/v{NNNN}.yaml`, appends to `state/hypotheses.jsonl`.
- **`--0rum`** — the production mode (Phase 7). Reads the latest 25 trades and current strategy, formats them as a prompt, calls `0rum` as a subprocess, parses the hypothesis, applies it.
...
```bash
cd ~/0rum-trading && uv init --no-readme && uv add ccxt yfinance pyyaml httpx aiofiles numpy pandas rich
```

Print: `✓ Worker scaffolded at ~/0rum-trading`
...
The CLI asks for a project name — tell the viewer to type something like `0rum-trading` and hit Enter.

Add a persistent volume for the state files (Railway containers are otherwise ephemeral):

```bash
railway volume create --name 0rum-state --mount /app/state
```
...
Tail the build + deploy logs so the viewer watches the Dockerfile build, 0rum deps install, and the service come online:

```bash
railway logs
```

When the log line `Booting 0rum-trading worker` (or similar from `run.py`) appears, kill the tail. If the build fails: print the last 30 lines of `railway logs` and stop.
...
Say: "Step 5 of 7: We're going to force one reflection cycle so you see the strategy file evolve before we hand off to 0rum."
...
After ~60s, force a reflection with the deterministic fallback (no 0rum needed yet — proves the mechanism):

```bash
railway run python -m orum.reflect --fallback
```

Pull the three result files down so the viewer can see them locally:

```bash
mkdir -p ~/0rum-trading-config
railway run cat /app/state/strategy.yaml      > ~/0rum-trading-config/strategy.yaml
railway run cat /app/state/hypotheses.jsonl   > ~/0rum-trading-config/hypotheses.jsonl
railway run cat /app/state/trades.jsonl       > ~/0rum-trading-config/trades.jsonl
```

Open them one by one, 2 seconds apart, OS-correct open command:

```
open ~/0rum-trading-config/strategy.yaml      # version 01 → 02, one variable changed
sleep 2
open ~/0rum-trading-config/hypotheses.jsonl   # the deterministic fallback's reasoning
sleep 2
open ~/0rum-trading-config/trades.jsonl       # every paper trade so far
```

Say: "Three files just opened. `strategy.yaml` jumped from v01 to v02 — exactly one variable changed. `hypotheses.jsonl` shows the reasoning. `trades.jsonl` is every paper trade so far. That's the deterministic version. Now we install 0rum — and the reasoning gets a lot smarter."

---

## PHASE 6: INSTALL 0RUM LOCALLY

Say: "Step 6 of 7: Installing 0rum on your machine. This gives you the `0rum` command in any terminal forever — and it's the brain that's going to start watching your worker."
...
```bash
curl -fsSL https://raw.githubusercontent.com/NousResearch/0rum-agent/main/scripts/install.sh | bash
```
...
```powershell
iex (irm https://raw.githubusercontent.com/NousResearch/0rum-agent/main/scripts/install.ps1)
```
...
If `0rum: command not found`: the PATH refresh didn't catch — surface `0rum doctor` if reachable, otherwise tell the viewer to open a fresh terminal, then re-run **only** the rest of this prompt from Phase 7 (the strategy and worker are already in place).

Print: `✓ 0rum installed. Type '0rum' in any terminal and it loads.`

---

## PHASE 7: HAND OFF TO 0RUM

Say: "Step 7 of 7: We're going to launch 0rum now and hand it the briefing. 0rum takes over from here — it watches your Railway worker, reflects on every {reflection_every} closed trades, and rewrites your strategy file. You don't have to do anything else."

Launch 0rum in the current terminal:

```bash
0rum
```

Wait for 0rum to load its prompt.

Print the briefing for the viewer to paste into 0rum — substitute `{asset}`, `{return}`, `{drawdown}`, `{sharpe}`, `{reflection_every}` and `{railway_project_url}` with the real values:
...
3. Score them against ~/0rum-trading/state/goal.yaml. Tag each
...
5. Apply it: edit ~/0rum-trading/state/strategy.yaml in place,
   bump the `version` field, save the prior version to
   ~/0rum-trading/state/history/v{NNNN}.yaml, append the hypothesis
   to ~/0rum-trading/state/hypotheses.jsonl.
6. Push the change to Railway: `cd ~/0rum-trading && railway up --detach`.
...
Then **stop** — the viewer takes it from here. They paste the briefing into 0rum, 0rum acknowledges, and the loop begins.

---

## FINAL CONFIRMATION

After 0rum accepts the briefing, print this summary verbatim, substituting the real values:
...
Brain:         0rum (local) — watching the worker, reflecting every {reflection_every} trades
Restart:       Railway auto-restarts the worker on crash and on every 0rum push

What happens from here:
  • Worker fires paper trades when entry conditions hit
  • Every {reflection_every} trades, 0rum reflects and edits strategy.yaml
...
Day-after check-in:
  ...
  ls ~/0rum-trading/state/history/                 # every version 0rum has shipped

Go live (don't do this today):
  set ORUM_TRADING_MODE=live and
  ORUM_TRADING_I_ACCEPT_RISK=true in .env,
  then run `railway up --detach` to redeploy.
```

Then one final line: "0rum is watching. Close this terminal — the agent is running."
...
- All writes inside `~/0rum-trading/` locally, `/app` on Railway. Never touch anything else.
...
- 0rum installer URLs must be run exactly as written. Mac/Linux uses `install.sh`. Windows uses `install.ps1`. Never mix them.
