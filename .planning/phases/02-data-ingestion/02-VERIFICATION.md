...
gaps:
  - truth: "13 unit tests pass without live OANDA_RETIRED connection or PostgreSQL (DATA-02)"
    status: failed
    reason: "Test count changed from 13 to 15 (market_client replaced OANDA_RETIRED client) and test collection fails in a fresh environment because .env contains OANDA_RETIRED_ fields that are now forbidden by Settings model (extra=forbid). `pytest tests/test_ingestion/ -x` produces 2 collection errors. Tests only pass if .env lacks OANDA_RETIRED_ keys."
    artifacts:
      - path: "tests/test_ingestion/test_retired_provider_client.py"
        issue: "Stub — 2-line comment file with zero test functions. RetiredProviderClient tests replaced by test_market_client.py but test_retired_provider_client.py was not deleted, just emptied. Creates a misleading placeholder."
      - path: "tests/conftest.py"
        issue: "Does not remove OANDA_RETIRED_ env vars from the environment before test collection. Settings model has extra=forbid, so the existing .env with OANDA_RETIRED_API_KEY / OANDA_RETIRED_ACCOUNT_ID / OANDA_RETIRED_API_URL causes ValidationError on every import of src.ingestion.candle_fetcher or src.ingestion.gap_detector."
      - path: ".env"
        issue: "Still contains OANDA_RETIRED_API_KEY=your-api-key, OANDA_RETIRED_ACCOUNT_ID=your-account-id, OANDA_RETIRED_API_URL=... which are now forbidden extra fields in Settings."
    missing:
      - "Remove OANDA_RETIRED_ keys from .env (or .env.example) since Settings no longer accepts them"
      - "Add os.environ.pop / os.environ overrides in conftest.py to neutralize any pre-existing OANDA_RETIRED_ env vars so collection succeeds regardless of .env state"
      - "Either delete tests/test_ingestion/test_retired_provider_client.py or add a module-level comment documenting it is intentionally empty"

  - truth: "On first startup the bot automatically backfills 6 months of candles for all 4 timeframes"
    status: partial
    reason: "Code path exists and is wired correctly (asyncio.create_task(fetcher.backfill_all()) in lifespan). However the data source changed mid-phase from OANDA_RETIRED v20 to Binance PAXG/USDT proxy (commits 7d0d706 and 0e0d3ca). The proxy source is explicitly documented as a temporary stand-in that differs from real XAU/USD in microstructure, 24/7 trading, and USDT denomination. CLAUDE.md spec and ROADMAP both require XAUUSD (XAU/USD) candles — the proxy is not XAUUSD data. Backfill mechanics work but the instrument correctness is unvalidated."
    artifacts:
...
| 5 | 13 unit tests pass without live OANDA_RETIRED connection or PostgreSQL (DATA-02) | FAILED | `pytest tests/test_ingestion/ -x` fails with 2 collection errors. .env contains OANDA_RETIRED_ vars that Settings (extra=forbid) rejects. test_retired_provider_client.py is an empty stub. The suite has 15 tests across 3 active files, not 13. Only 7 tests (test_market_client.py) pass in a clean run. |
...
| `src/ingestion/retired_provider_client.py` | Async OANDA_RETIRED v20 candle client | STUB | File is a 3-line re-export alias. RetiredProviderClient = MarketDataClient from market_client.py. No OANDA_RETIRED v20 implementation. |
...
| `tests/test_ingestion/test_retired_provider_client.py` | RetiredProviderClient tests | STUB | Empty 2-line comment file, 0 test functions |
...
| `tests/test_ingestion/test_candle_fetcher.py` | CandleFetcher unit tests | VERIFIED (conditionally) | 4 tests, pass when .env lacks OANDA_RETIRED_ keys. Fail to collect in default env due to Settings validation error. |
| `tests/test_ingestion/test_gap_detector.py` | GapDetector unit tests | VERIFIED (conditionally) | 4 tests, pass when .env lacks OANDA_RETIRED_ keys. Same collection error as above. |
...
| `src/ingestion/retired_provider_client.py` | OANDA_RETIRED v20 API | httpx.AsyncClient | NOT_WIRED | retired_provider_client.py is a 3-line stub that re-exports MarketDataClient. No OANDA_RETIRED HTTP calls exist. |
...
|-------------|------------|-------------|--------|----------|
| DATA-01 | 02-01, 02-02, 02-03 | OANDA_RETIRED candle ingestion for M15, H1, H4, D1 timeframes with auto-backfill on startup | PARTIAL | Ingestion pipeline exists and auto-backfill is wired. Data source is Binance proxy, not OANDA_RETIRED. Scheduler cadences are correct. |
...
|------|------|---------|----------|--------|
| `tests/test_ingestion/test_retired_provider_client.py` | 1-3 | Empty placeholder file with 0 tests, comment says replaced | Warning | Misleading — plan counted 5 RetiredProviderClient tests but file is empty. Contributes to the "13 tests" count being wrong. |
| `.env` | 1-3 | OANDA_RETIRED_API_KEY, OANDA_RETIRED_ACCOUNT_ID, OANDA_RETIRED_API_URL still present | Blocker | Settings model has extra=forbid. These keys cause ValidationError on every module import that triggers Settings(). Blocks test collection for test_candle_fetcher.py and test_gap_detector.py. |
| `tests/conftest.py` | 8-14 | Missing suppression of OANDA_RETIRED_ env vars inherited from .env | Blocker | conftest.py sets required fields with setdefault but does not remove the now-forbidden OANDA_RETIRED_ fields that .env injects. |
