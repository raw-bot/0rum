...
[testing paused — 8 items outstanding, blocked on OANDA API activation]
...
result: pass
note: OANDA 401 on backfill expected (no valid creds in test env) — server did not crash, error was caught and logged via structured JSON logger

### 2. Unit Tests Pass (Offline)
expected: Running `pytest tests/test_ingestion/ -x -v` with no live OANDA connection or PostgreSQL passes all 13 tests — 5 OandaClient, 4 CandleFetcher, 4 GapDetector. No failures, no collection errors.
result: skipped
reason: user will test later when OANDA API is activated

### 3. OANDA Client Candle Fetch
expected: Calling `OandaClient.get_candles(instrument="EUR_USD", granularity="H1", count=10)` returns a list of raw candle dicts. D1 granularity maps to "D" when sent to OANDA. Providing `from_time` removes `count` from the request params automatically.
result: blocked
blocked_by: third-party
reason: "OANDA API not yet activated — will test once API credentials are active"
...
result: blocked
blocked_by: third-party
reason: "OANDA API not yet activated"
...
result: blocked
blocked_by: third-party
reason: "OANDA API not yet activated — no candle data in DB to prune"
...
result: blocked
blocked_by: third-party
reason: "OANDA API not yet activated — no candle data in DB to scan"
