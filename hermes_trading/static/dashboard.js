"use strict";
// Hermes Ops Console — renders /api/state into a dense operational view with a
...
/* ---- indicators computed client-side (parity with Hermes) ------------- */
...
function drawPriceChart() {
  ...
  // executed trades (IN -> OUT) — what Hermes actually did
  if (P.filter !== "signals") {
    mk.forEach((m) => {
...
function renderPrice(s) {
  ...
  setPriceData(s);
  if (P.series.length < 2) { card.innerHTML = `<h2>Trade Signals · Pine → Hermes</h2><div class="flat">waiting for price history…</div>`; P.init = false; return; }
  if (!P.init) {
    const fbtn = (f, t) => `<button data-f="${f}" class="${P.filter === f ? "on" : ""}">${t}</button>`;
    card.innerHTML = `
      <h2>Trade Signals · Pine → Hermes <span class="hint" id="px-hint"></span>
...
async function postWorker(path) {
  ...
  if (!r.ok) {
    ...
    const hint = r.status === 404 ? " — redémarre le serveur dashboard (python -m hermes_trading.dashboard)" : "";
...
async function tick() {
  try {
    ...
    renderPosition(s); renderEquity(s); renderStrategy(s); renderLeverage(s); renderExternal(s); renderLogs(s); renderTrades(s);
