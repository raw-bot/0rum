...
PYTHONPATH=. .venv/bin/python scripts/run_replay_harness.py regime --run-id regime_v1 \
    --runs iso_ak,iso_utbot,iso_ha
    # détecteur de régime en SHADOW (scripts/replay_harness/regime.py) : features
    # ex ante (pente canal/ATR + ER Kaufman) au ts d'entrée de chaque trade,
    # seuils appris en walk-forward puis FIGÉS, verdicts journalisés — jamais
    # branché sur le live. Rapport : backtests/reports/chantier2_regime.md.
# NOTE : le forecast gate a été SUPPRIMÉ le 2026-07-17 (verdict de l'étude OOS,
# backtests/reports/chantier3_forecast_gate.md). Les artefacts gate_study_v1 et
# les runs de référence antérieurs (champ `gate` dans leurs outcomes) restent
# valables : le gate n'a jamais influencé une décision (verrouillé 310/311).
