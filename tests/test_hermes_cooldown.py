...
from hermes_trading.reflect import _cooldown_status, _hermes
...
class HermesCooldownTests(unittest.TestCase):
    ...
    def test_hermes_mode_does_not_call_the_llm_during_cooldown(self):
        hypotheses = [{"ts": "2026-06-02T14:07:17+00:00", "changed": True, "variable": "entry.threshold"}]
        trades = [{"ts": "2026-06-02T14:36:42+00:00", "pnl_pct": -0.001}]

        with patch("hermes_trading.reflect.subprocess.run") as run:
            hypothesis = _hermes({}, GOAL, trades, hypotheses)

        run.assert_not_called()
        self.assertFalse(hypothesis["changed"])
        self.assertEqual(hypothesis["mode"], "hermes")
