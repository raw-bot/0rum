...
- [ ] Create `/Applications/0rum/.sandbox/0rum-one-shot-home`.
- [ ] Set `HOME=/Applications/0rum/.sandbox/0rum-one-shot-home` for the replay process.
...
- [ ] Confirm no global 0rum install will run during the first replay.
...
- [ ] Stop if the agent tries to write outside `/Applications/0rum/.sandbox/0rum-one-shot-home`.
...
- [ ] Stop if the agent tries to install 0rum globally before the generated worker is audited.
- [ ] Stop if Railway deploy uses an existing production project.

## Required Paper-Mode Invariants

- [ ] `.env` has `ORUM_TRADING_MODE=paper`.
- [ ] `.env` has `ORUM_TRADING_I_ACCEPT_RISK=false`.
...
- [ ] List all files created under `/Applications/0rum/.sandbox/0rum-one-shot-home`.
...
- [ ] Confirm 0rum has not been started as a persistent autonomous writer.

## Promotion Gate

Do not connect 0rum cron until all of these are true:

- [ ] Worker has run in paper mode for at least one clean test session.
- [ ] Generated code has been reviewed.
- [ ] Dependencies have been reviewed.
- [ ] A read-only 0rum briefing exists.
