...
| Broker | OANDA v20 API (practice → live) |
...
├─────────────────────────────────────────────────────┤
│               EXECUTION ENGINE (L6)                  │
│     Mode signal (Telegram) │ Mode auto (OANDA)       │
├─────────────────────────────────────────────────────┤
│               RISK MANAGEMENT (L5)                   │
...
├─────────────────────────────────────────────────────┤
│             DATA INGESTION (L1)                      │
│   OANDA feed · 4 timeframes · PostgreSQL             │
└─────────────────────────────────────────────────────┘
```
...
│   ├── ingestion/
│   │   ├── __init__.py
│   │   ├── oanda_client.py        # OANDA v20 async client
...
│   │   ├── oanda_executor.py      # OANDA order placement
...
OANDA_API_URL=https://api-fxpractice.oanda.com  # practice
...
class OandaClient:
    """Client async pour OANDA v20 API."""
    ...
    async def get_candles(
        self,
        instrument: str,
        granularity: str,  # M15, H1, H4, D
        count: int = 500,
        from_time: str | None = None,
    ) -> list[dict]:
        """Fetch candles from OANDA v20."""
        # granularity mapping: D1 → D pour OANDA
...
- L'utilisateur décide manuellement d'exécuter ou non via OANDA
...
Le pipeline exécute automatiquement via OANDA API :
...
class ExecutionRouter:
    def __init__(self, settings: Settings, oanda: OandaExecutor, telegram: SignalSender):
    ...
    async def execute(self, signal: ApprovedSignal, size: float):
        if self.mode == ExecutionMode.SIGNAL:
            ...
            await self.oanda.place_order(signal, size)
...
- ❌ **Pas de dépendance API externe** (hors OANDA) : pas de Fear & Greed index, pas de news API
...
- Mode auto : OANDA order executor + partial close + trailing stop
...
# Orders
POST /v3/accounts/{accountId}/orders        # créer un ordre
GET  /v3/accounts/{accountId}/orders         # lister les ordres

# Trades
GET  /v3/accounts/{accountId}/trades         # lister les trades
PUT  /v3/accounts/{accountId}/trades/{id}    # modifier SL/TP
PUT  /v3/accounts/{accountId}/trades/{id}/close  # close partiel/total

# Pricing
GET /v3/accounts/{accountId}/pricing?instruments=XAU_USD  # prix live
```

### Notes OANDA

- Instrument OANDA pour le Gold : `XAU_USD` (pas `XAUUSD`)
- Granularités : `M15`, `H1`, `H4`, `D` (pas `D1`)
- Le format datetime est RFC3339 : `2024-01-15T00:00:00.000000000Z`
- Auth header : `Authorization: Bearer {token}`
- Rate limiter : max 120 requests/seconde (en pratique, 1 req/500ms suffit)
