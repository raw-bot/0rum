# Rapport d'audit — 0rum (paper-mode self-improving trading worker)

## 1. Résumé exécutif

Ce que fait l'application : un agent de trading papier (aucun ordre réel) sur BTC/USDT. Un worker interroge Binance toutes les 60 s, calcule un RSI 14 sur bougies 1 minute, ouvre/ferme des positions simulées (mean-reversion long-only), et journalise tout dans des fichiers (state/*.jsonl, *.yaml, *.json). Un « cerveau » de réflexion (LLM Gemini/Ollama via le CLI externe 0rum, ou un fallback déterministe) ajuste une variable de stratégie tous les 10 trades fermés. Un dashboard HTTP local (127.0.0.1:8787) visualise l'état. Le code vit dans .sandbox/0rum-one-shot-home/0rum-trading/ — c'est le produit d'un « replay sandbox » d'un prompt one-shot, documenté dans sandbox-plan/.
...
│                       │  fallback offline                                                                                                  │
├───────────────────────┼────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┤
│ reflect.py            │ CLI de réflexion : --fallback (déterministe) ou --0rum (subprocess vers le CLI LLM 0rum) ; mute strategy.yaml  │
├───────────────────────┼────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┤
│ orum_watch.py       │ Watcher (boucle 1800 s) : déclenche reflect --0rum tous les 10 trades fermés                                     │
├───────────────────────┼────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┤
...
```text
 ...
 (fallback offline) └──────┬──────────────┬───────────┬───────────┘
            ...
            strategy.yaml ◀── reflect.py ◀── orum_watch.py (toutes les 1800s)
                  │  ▲          │   ▲
                  │  └──────────┘   └── CLI `0rum` → LLM (Gemini / Ollama)
                  ▼
...
État: Fonctionnelle (testée) mais jamais branchée au runtime — le watcher n'appelle que --0rum
Chemin d'exécution: reflect._fallback
Risque principal: cooldown/évidence existent ici mais pas en mode 0rum
Certitude: Haute

**Fonctionnalité: Réflexion LLM (0rum)**
État: Fonctionnelle mais fragile
Chemin d'exécution: orum_watch.maybe_reflect_once → reflect._0rum
...
**Priorité: P2**
Problème: Course entre deux réflexions : le watcher et POST /api/reflect (bouton UI, aucun rate-limit) peuvent lancer reflect --0rum simultanément
Fichier / zone: dashboard.py:333-349 + orum_watch.py:88
...
Fichier / zone: dashboard.py:264-266 vs orum_watch.py:61
...
**Priorité: P3**
Problème: Cooldown et exigence d'évidence (cooldown_after_change_trades, evidence_reflections_required) implémentés uniquement dans _fallback, jamais en mode 0rum — qui est le seul mode branché au watcher
Fichier / zone: reflect.py:77-118 vs _0rum
...
Utilisée ?: Non branchée (le watcher n'appelle que --0rum)
...
- Timestamps : cohérents (UTC isoformat partout, datetime.now(UTC)), comparaisons lexicographiques de chaînes ISO (reflect._trades_after, orum_watch._trades_after) — correctes tant que le format reste identique, fragiles si un « Z » apparaît. candle_ts Binance en ms epoch, mais "offline-59" (chaîne) en mode fallback : type instable sur le même champ.
...
- Clé API en clair : .sandbox/0rum-local-llm-home/.gemini_api_key contient une clé lue par orum_watch.py:84-86 et injectée dans l'environnement. Un auth.json existe aussi. Le projet n'est pas un dépôt git (pas de risque de commit immédiat), mais la clé vit dans l'arborescence du projet, sans permissions restreintes vérifiées. (Valeurs non reproduites ici.)
...
- reflect._0rum passe le prompt entier en argument de ligne de commande (0rum -z <prompt>) : visible dans ps par tout utilisateur local, et le flag --ignore-rules désactive explicitement des garde-fous du CLI 0rum.
...
- Heartbeats : bonne idée (heartbeat.json, orum_watcher.json, avec sources de données et raison de décision — réellement utile), mais écrasés à chaque écriture : aucun historique, impossible de diagnostiquer a posteriori « que voyait le bot à 14 h 32 ».
...
6. Écritures atomiques (tempfile + os.replace) pour strategy.yaml, open_position.json, heartbeats — loop.py, reflect.py, orum_watch.py. Résout : lectures partielles, corruption au crash. Effort : faible.
...
10. Appliquer cooldown + évidence au mode 0rum (réutiliser la logique de _fallback comme garde autour de _0rum). Effort : moyen.
...
- Le CLI 0rum lui-même : binaire externe non audité ici (seuls ses fichiers de config/sessions dans .sandbox/0rum-local-llm-home/ sont visibles). Le comportement exact de --ignore-rules et la robustesse de sa sortie JSON ne sont pas vérifiables depuis ce code.
...
- Comportement de 0rum -z sous prompt long : le passage du prompt en argv peut atteindre des limites de taille avec un gros historique de trades — non testé.
...
│     │ watcher/endpoint POST                                                          │ orum_watch.py, dashboard.py  │                    │
├─────┼────────────────────────────────────────────────────────────────────────────────┼────────────────────────────────┼────────────────────┤
...
├─────┼────────────────────────────────────────────────────────────────────────────────┼────────────────────────────────┼────────────────────┤
│ 8   │ Cooldown + évidence appliqués aussi au mode 0rum (la logique existe déjà     │ reflect.py                     │ Faible             │
│     │ dans _fallback, il suffit de la mettre en garde-fou autour de _0rum)         │                                │                    │
└─────┴────────────────────────────────────────────────────────────────────────────────┴────────────────────────────────┴────────────────────┘
