...
ROOT="$HOME/hermes-trading"

mkdir -p "$ROOT/hermes_trading/adapters" "$ROOT/state/history" "$ROOT/tests"
...
cat > "$ROOT/hermes_trading/__init__.py" <<'PY'
__all__ = ["__version__"]
__version__ = "0.1.0"
PY

cat > "$ROOT/hermes_trading/adapters/__init__.py" <<'PY'
...
cat > "$ROOT/hermes_trading/adapters/price.py" <<'PY'
...
cat > "$ROOT/hermes_trading/adapters/onchain.py" <<'PY'
from __future__ import annotations
...
cat > "$ROOT/hermes_trading/adapters/news.py" <<'PY'
from __future__ import annotations
...
cat > "$ROOT/hermes_trading/adapters/macro.py" <<'PY'
from __future__ import annotations
...
cat > "$ROOT/hermes_trading/score.py" <<'PY'
from __future__ import annotations
...
cat > "$ROOT/hermes_trading/loop.py" <<'PY'
from __future__ import annotations
...
ROOT = Path(os.getenv("HERMES_TRADING_ROOT", Path.cwd()))
STATE = ROOT / "state"
console = Console()
...
async def run(asset: str) -> None:
    console.print(f"Booting hermes-trading worker for {asset} in paper mode")
    failures = 0
    while True:
        try:
...
cat > "$ROOT/hermes_trading/run.py" <<'PY'
from __future__ import annotations
...
cat > "$ROOT/hermes_trading/reflect.py" <<'PY'
from __future__ import annotations
...
def _hermes_change(strategy: dict, trades: list[dict], goal: dict) -> tuple[dict, dict]:
...
"""
    result = subprocess.run(["hermes", "--print", prompt], text=True, capture_output=True, check=True)
    ...
    if variable == "entry.threshold":
        ...
        raise ValueError(f"unsupported variable from hermes: {variable}")
    hypothesis.update({"mode": "hermes", "before": before})
...
def reflect(mode: str) -> dict:
    goal = _load_yaml(STATE / "goal.yaml")
    strategy = _load_yaml(STATE / "strategy.yaml")
    trades = _load_trades(25 if mode == "hermes" else None)
    if not trades:
        trades = [{"pnl_pct": -0.001, "status": "synthetic_seed"}]
    _archive(strategy)
    updated, hypothesis = _fallback_change(strategy, trades, goal) if mode == "fallback" else _hermes_change(strategy, trades, goal)
...
def main(argv: list[str] | None = None) -> None:
    ...
    args = parser.parse_args(argv)
    print(json.dumps(reflect("fallback" if args.fallback else "hermes"), indent=2, sort_keys=True))
...
from hermes_trading.reflect import reflect
from hermes_trading.score import max_drawdown, score
