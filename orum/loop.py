...
from orum.accounting import compound_balance, max_drawdown
from orum.adapters import macro, news, onchain, price
from orum.adapters.base import require_schema
from orum.dsl import evaluator as dsl_evaluator
from orum.dsl import indicators as dsl_indicators
...
from orum.paths import HEARTBEAT_PATH, STATE_DIR, STRATEGY_PATH, TRADES_PATH
...
def _shadow_check_native_dsl(strategy: dict, candles: list[dict], asset: str, entry_eval: dict, exit_eval: dict) -> None:
    """Logs (never raises, never changes a decision) when NativeDslEngine
    disagrees with the entry_signal_fired/exit_signal_fired results computed
    above. Proves the wrapper is equivalent on live data before any cutover
    relies on it -- see orum/strategies/native_dsl.py. Imports locally to
    avoid a loop<->strategies import cycle (native_dsl imports
    strategy_direction from this module), the same reason executor.py's
    imports are local."""
    ...
    try:
        ...
        signal = engine.on_candle(candles[-1], StrategyContext(candles=candles, symbol=asset, timeframe="1m"))
    except Exception as exc:  # noqa: BLE001 - a shadow check must never affect the real decision path
        log_event("strategy_engine_shadow_error", f"NativeDslEngine raised during shadow check: {exc}", asset=asset)
        return

    legacy = "exit" if exit_eval["triggered"] else ("long" if entry_eval["triggered"] else None)
    shadow = signal.side.value if signal else None
    if legacy != shadow:
        log_event(
            "strategy_engine_shadow_disagreement",
            f"NativeDslEngine ({shadow}) disagrees with entry/exit_signal_fired ({legacy})",
            asset=asset,
            legacy=legacy,
            shadow=shadow,
        )
...
def worker_outage_detected(*, now: datetime | None = None, max_gap_seconds: float | None = None) -> bool:
    ...
    if max_gap_seconds is None:
        max_gap_seconds = float(os.getenv("0RUM_OUTAGE_GAP_MINUTES", "15")) * 60.0
...
def open_position_from_signal(
    ...
    return {
        ...
        **sizing,
        "mode": os.getenv("ORUM_TRADING_MODE", "paper"),
    }
...
def _build_closed_trade(position: dict, strategy: dict, market: dict, rsi: float | None, regime: dict | None, exit_reason: str, fill_price: float | None = None) -> dict:
    ...
    return {
        ...
        "mode": position.get("mode", os.getenv("ORUM_TRADING_MODE", "paper")),
...
async def _maybe_backfill_live_candles(asset: str) -> None:
    ...
    try:
        ...
        from orum.dsl.backtest import load_history
...
async def run_loop(goal: dict, *, iterations: int | None = None) -> None:
    ...
    from orum.external.orchestrator import signal_source

    STATE_DIR.mkdir(parents=True, exist_ok=True)
    executor: "PaperExecutor" = PaperExecutor()
    log_event("worker_boot", "Booting 0rum-trading worker", asset=goal.get("asset", "BTC/USDT"))
    ...
    interval = int(os.getenv("0RUM_LOOP_INTERVAL_SECONDS", "60"))
    ...
    while True:
        try:
            ...
            if not offline:
                # Offline freezes entry_signal_fired but NativeDslEngine has no
                # offline check of its own (that guard is the caller's job, see
                # native_dsl.py) -- comparing while offline would log a
                # spurious "disagreement" that isn't a wrapper defect.
                _shadow_check_native_dsl(strategy, candles, asset, entry_eval, exit_eval)
            groups = strategy_dsl_groups(strategy)
