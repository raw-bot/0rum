"""Phase 5: MCP polling transport — TradingView -> 0rum, no webhook.

The free plan cannot POST outbound webhooks, so instead of TradingView pushing
to us, 0rum PULLS: it reads the 0rum Mirror's debug table over the MCP
...
it only transports a candidate into the pipeline, where 0rum judges it.
"""
...
from orum.external.orchestrator import ExternalOrchestrator, OrchestratorOutcome
...
def extract_mirror_payload(
    response: dict, *, study_name: str = "0rum Mirror", payload_key: str = "payload"
) -> str | None:
