"""AK MACD bridge — the P1 polling binding (TradingView -> 0rum), shadow-first.
...
from orum.paths import STATE_DIR, GOAL_PATH, TRADES_PATH
from orum.external.poller import extract_mirror_payload
from orum.external.signal import parse_external_signal, ExternalSignalError
...
def _default_state() -> dict:
    return {
        ...
        "trading_mode": os.getenv("ORUM_TRADING_MODE", "paper"),
        "price_offline": False,
    }
...
def main(argv: list[str] | None = None) -> None:
    p = argparse.ArgumentParser(description="AK MACD TradingView->0rum polling bridge")
